...
```mdx title="标题式步骤组规则"
1. 不支持步骤组嵌套, 因为FumaDocs底层暂不支持, 重写代价大, 并且从DDaaS角度, 如果需要步骤组嵌套, 说明文档不够简练、不容易懂
2. 命名规则规则: 多个`#` + `空格` + `步骤编号数字` + `英文.` + `空格` + `标题名称`, 例如: `### 1. 三级标题的步骤1`
3. 不满足规则的, 会自动降级为普通标题; 步骤编号也由文档指定, 底层解析不会自动递增编号
```

<Tabs items={["魔改FumaDocs源码"]}>
  <Tab value="魔改FumaDocs源码">
    ### 1. Fork源码
    - `git`, 注意它是一个多包工程
    - `pnpm build`, 会出现很多报错, 逐一解决, 删除无用代码
    - `pnpm --filter=docs dev`, 启动项目, 验证修改

    ### 2. MDX解析流程分析
    - `source.ts`, 定义数据源和路由
    - `source.config.ts`, 定义元数据解析规则, 插件链
    - `mdx-components.tsx`, 指定渲染结果
    - `mdx`插件链, 这里很容易埋坑, 因为插件顺序会影响解析结果
    - `remark-heading`->`remark-mdx-steps`->

    ### 3. 关键词定位
    - 关键词定位, 需要从`source.config.ts`入手
